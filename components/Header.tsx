...
export const Header: React.FC<HeaderProps> = ({ scrollToId, darkMode, toggleTheme }) => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-bairro-cream/95 dark:bg-slate-900/95 backdrop-blur-sm shadow-sm border-b border-orange-100 dark:border-slate-800 transition-colors duration-300 py-4">
      <div className="w-full max-w-[1200px] mx-auto px-5 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex w-full md:w-auto justify-between items-center">
            <div className="flex items-center gap-2.5 text-bairro-teal dark:text-primary-light text-2xl font-serif font-black cursor-pointer hover:opacity-80 transition-opacity" onClick={() => { setCurrentView('home'); window.scrollTo({ top: 0, behavior: 'smooth' }) }}>
            <HandHeart size={32} strokeWidth={2.5} />
            <span className="hidden sm:inline">Bairro</span>
            </div>
        ...
        <div className={`${mobileMenuOpen ? 'flex' : 'hidden'} md:flex flex-col md:flex-row items-center gap-6 w-full md:w-auto`}>
            ...
            <ul className="flex flex-col md:flex-row justify-center items-center gap-2 md:gap-1">
                {navItems.map((item) => (
                    ...
                    <button
                        ...
                        className="w-full md:w-auto px-3 py-2 rounded-lg text-sm font-serif text-bairro-stone-600 dark:text-slate-400 hover:text-bairro-teal dark:hover:text-primary-light hover:bg-orange-100 dark:hover:bg-primary/10 transition-colors"
                    >
                        {item.label}
                    </button>
                    </li>
            ...
            <button className="hidden md:block px-6 py-2 bg-bairro-amber text-white font-bold rounded-full shadow-sm hover:bg-amber-700 transition-all text-sm">
